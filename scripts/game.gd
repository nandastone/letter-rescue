...
func _prepare_original_initialization() -> void:
	...
	if original_start_seed < 0:
		original_start_seed = preload("res://scripts/wr1_startup.gd").seed_from_dos_time(Time.get_datetime_dict_from_system())
	var starts: Array = []
...
func _show_original_level_title(initial: bool) -> void:
	...
	original_level_title = preload("res://scripts/wr1_level_title.gd").new()
...
func _on_original_presented(state: RefCounted) -> void:
	...
	if original_reward != null and original_gruzzles != null:
		...
		if not draw.is_empty():
			...
			preload("res://scripts/wr1_text.gd").draw(raster, draw.text, Vector2i(20,8), draw.color, Color.TRANSPARENT)
			if draw.has("caption"):
				preload("res://scripts/wr1_text.gd").draw(raster, draw.caption, Vector2i.ZERO, draw.color, Color.TRANSPARENT)
			...
			preload("res://scripts/wr1_clear_text.gd").reward(original_reward_sprite, raster, draw.get("caption", ""), draw.text, draw.color)
		preload("res://scripts/wr1_clear_text.gd").sync_sprite(original_reward_sprite)
...
func _on_original_moved(state: RefCounted) -> void:
	...
	if original_door_state != 0 and preload("res://scripts/wr1_exit.gd").touches(state.gx, state.gy, door_grid):
...
func begin_original_recap() -> bool:
	...
	player.original_recap = preload("res://scripts/wr1_recap.gd").new()
	...
	var scene_work = preload("res://scripts/wr1_scene_work.gd").new()
	...
	original_recap_view = preload("res://scripts/wr1_recap_view.gd").new()
...
func _collect_original_cell(col: int, row: int) -> void:
	...
	for child in entities.get_children():
		if child.get_script() == preload("res://scripts/collectible.gd") and child.type == pickup_type and child.position == Vector2(tile * 16):
			child.queue_free()
...
func save_original_profile() -> void:
	...
	preload("res://scripts/wr1_profiles.gd").save_player(GameManager.original_player_name,profile)
	preload("res://scripts/wr1_profiles.gd").update_high_score(GameManager.original_player_name,GameManager.score)
...
func draw_original_word_list(image: Image) -> void:
	if original_words == null:
		return
	var renderer = preload("res://scripts/wr1_frontend_render.gd")
	image.blit_rect(renderer.asset("HELP.WR"),Rect2i(192,0,128,192),Vector2i(192,0))
	for i in range(7):
		var picture: Texture2D = preload("res://scripts/wr1_pictures.gd").load_picture(original_words.words[i])
