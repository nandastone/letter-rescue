...
const Controls = preload("res://scripts/wr1_controls.gd")
var state := "menu"
var index := 0
var selection := 0
...
func redraw() -> void:
	Render.begin_reading(preload("res://scripts/wr1_clear_text.gd").enabled())
	var result: Image
	match state:
	...
	if preload("res://scripts/wr1_clear_text.gd").enabled():
		result = preload("res://scripts/wr1_clear_text.gd").word_list(self, result)
		preload("res://scripts/wr1_clear_text.gd").frontend_labels(self, Render.reading_runs)
	Render.reading = false
...
func page_context() -> Dictionary:
	...
	if game != null and game.original_words != null:
		...
		var initial := preload("res://scripts/wr1_words.gd").new()
		initial.next_words()
		words = initial.words
