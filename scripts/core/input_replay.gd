...
func _start_replay(path: String = "") -> void:
	...
	if not original_music_clock.is_empty() and LaunchArgs.legacy():
		frontend_clock = load("res://scripts/legacy/wr1_frontend_clock.gd").new()
		frontend_clock.configure(original_music_clock)
	original_video_start = data.get("original_video_start", {})
	if LaunchArgs.legacy() and not load("res://scripts/legacy/wr1_video_clock.gd").valid_timing(original_video_start):
