...
func set_original_picture(picture_word: String) -> void:
	...
	preload("res://scripts/core/wr1_text.gd").draw(card, word.to_lower(), Vector2i(32 - 4 * word.length(), 4), Color.BLACK, Color.WHITE)
	...
	preload("res://scripts/core/wr1_clear_text.gd").sprite_text(original_word_sprite, card, [
...
func update_visual() -> void:
	...
	if original_word_sprite != null:
		preload("res://scripts/core/wr1_clear_text.gd").sync_sprite(original_word_sprite)
	preload("res://scripts/core/wr1_clear_text.gd").sync_sprite($Sprite2D)
