...
func present_original_frame(frame: int, bonus: bool, flash: int) -> void:
	...
	if bonus:
		...
		preload("res://scripts/core/wr1_text.gd").draw(raster, "Bonus #2", Vector2i.ZERO, ink, Color.TRANSPARENT)
		preload("res://scripts/core/wr1_text.gd").draw(raster, "500", Vector2i(20,8), ink, Color.TRANSPARENT)
		original_bonus_sprite.texture = ImageTexture.create_from_image(raster)
		original_bonus_sprite.position = Vector2(-20,-8)
		preload("res://scripts/core/wr1_clear_text.gd").reward(original_bonus_sprite, raster, "Bonus #2", "500", ink)
