...
func update_visual() -> void:
	match type:
		...
		"book":
			...
			if preload("res://scripts/core/wr1_clear_text.gd").enabled() and not has_node("ClearBook"):
				...
				add_child(cover)
				preload("res://scripts/core/wr1_clear_text.gd").label(cover, Rect2(-1,-2,14,10), "book", Color8(255,255,85), 5)
		"letter":
			var atlas := AtlasTexture.new()
			...
			$Sprite2D.position = Vector2.ZERO
			$Sprite2D.show()
			if preload("res://scripts/core/wr1_clear_text.gd").enabled() and not has_node("ClearLetter"):
				preload("res://scripts/core/wr1_clear_text.gd").configure_letter(self)
