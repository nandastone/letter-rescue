...
const WEB_FLAGS := ["legacy", "skip-intro", "mute-original-audio", "aspect-43"]
...
## than wide, and round objects are drawn wider than tall to suit -- but the
## geometric art is not, so 4:3 turns the 24x24 question blocks into tall
## rectangles. Square pixels are the default; --aspect-43 asks for the
## original display shape.
static func square_pixels() -> bool:
	return legacy() or not "--aspect-43" in _args
