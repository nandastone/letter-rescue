...
# Legacy mode retains the reference chrome. Original mode updates those
# pixels with the recovered BIOS text and picture blits.
...
func show_message(text: String, duration: float = 2.0) -> void:
	if original_rules:
		return # Native events update the chrome; no extra playfield text box.
