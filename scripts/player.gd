...
func configure_original(data: Dictionary, restarting: bool = false, advancing: bool = false) -> void:
	get_window().title = "Letter Rescue - Original Rules"
	...
	if first_configuration:
		...
		if not InputReplay.original_music_clock.is_empty():
			original_music_clock = preload("res://scripts/wr1_music_clock.gd").new()
...
func _present_original(queue_video: bool = true) -> void:
	...
	facing_right = original_state.facing == 0
	on_ladder = original_state.climbing
...
func _original_physics(delta: float) -> void:
	...
	original_state.render_step()
	_present_original()
	velocity = (global_position - before) / WR1Motion.STEP_SECONDS
	if get_parent().original_door_state == 2:
		...
		original_exit = preload("res://scripts/wr1_exit.gd").new()
...
func die() -> void:
	...
	if original_rules_enabled:
		original_rescue = preload("res://scripts/wr1_rescue.gd").new()
