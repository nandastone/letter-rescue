...
static func reading_asset(image: Image, name: String, x: int, y: int) -> void:
	if reading:
		reading_runs.append_array(preload("res://scripts/wr1_clear_pages.gd").apply(image, name, Vector2i(x,y)))
...
static func illustration(image: Image, command: Dictionary, context: Dictionary) -> void:
	...
	match command.kind:
		...
		"picture":
			var word: String = context.get("words",["toe","pot"])[int(command.slot)]
			source = preload("res://scripts/wr1_pictures.gd").load_picture(word).get_image()
